...
from opensac.models import Mechanisms, Session
...
class BrokerSession:
    ...
    def cache_content(self, identity: str, row: dict[str, Any], budget: int) -> None:
        ...
        if not identity or identity in self.content_cache:
            return
    ...
    def remember(
        ...
        *,
        ...
        requested_aliases = {handle.source, *(aliases or set())}
        if record is not None:
            for alias in requested_aliases:
                self._remember_alias(record, alias)
            return record.handle.source
